...
//  }
  ...
  public WalletClient() throws  CipherException {
    byte[] password =new  byte[]{80,97,115,115,119,48,114,100,49,50,51,52,53};
    ECKey ecKey = new ECKey(Utils.getRandom());
    this.walletFile = Wallet.createStandard(password, ecKey);
    this.address = ecKey.getAddress();
  }
  ...
  public WalletClient(byte[] priKey) throws CipherException {
    ECKey ecKey = ECKey.fromPrivate(priKey);
    this.eckey = ecKey;
    this.address = ecKey.getAddress();
  }
...
//  }
  ...
  public  Transaction createAccountTransaction(byte[] owner, byte[] address) {
